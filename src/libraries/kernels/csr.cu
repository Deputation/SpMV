...
__global__ void spmv_csr_naive_fake(const int64_t *csr_arow,
  ...
  for (int64_t j = csr_arow[idx]; j < csr_arow[idx + 1]; j++) {
    // everything has to be volatile here
    volatile int64_t acol_read = acol[j];
    volatile valuetype aval_read = aval[j];
...
__global__ void spmv_csr_warp_row(const int64_t *csr_arow, const int64_t *acol,
  ...
  int64_t assigned_row = idx / WARP_SIZE;
  // we will need to identify each thread in the warp
  int64_t tid_in_warp = threadIdx.x % WARP_SIZE;
  ...
  int64_t csr_arow_start = csr_arow[assigned_row];
  int64_t csr_arow_end = csr_arow[assigned_row + 1];
  ...
  for (int64_t j = csr_arow_start + tid_in_warp; j < csr_arow_end;
       j += WARP_SIZE) {
