...
void allocate_device_buffers(int64_t *host_arow, int64_t *host_acol,
                             valuetype *host_aval, int64_t nnz,
                             int64_t **dev_acol, int64_t **dev_arow,
                             valuetype **dev_aval) {
  zero_check(cudaMalloc(dev_arow, sizeof(int64_t) * nnz));
  zero_check(cudaMalloc(dev_acol, sizeof(int64_t) * nnz));
  zero_check(cudaMalloc(dev_aval, sizeof(valuetype) * nnz));

  zero_check(cudaMemcpy(*dev_arow, host_arow, sizeof(int64_t) * nnz,
...
int main(int argc, const char *argv[]) {
  ...
  int64_t *dev_acol, *dev_arow;
  valuetype *dev_aval;
  allocate_device_buffers(host_arow, host_acol, host_aval, nnz, &dev_acol,
                          &dev_arow, &dev_aval);
  ...
  benchmark_results results[3];

  bench_expression(results[0] = benchmark_coo_cpu(host_arow, host_acol,
                                                  host_aval, rows, cols, nnz,
                                                  &dev_out_coo, dev_dense_vec));
  bench_expression(
      results[1] = benchmark_coo_kernel_naive(
          dev_arow, dev_acol, dev_aval, rows, nnz, dev_out_coo, dev_dense_vec));

  bench_expression(results[2] = benchmark_cusparse_coo_gpu(
                       handle, dev_arow, dev_acol, dev_aval, rows, cols, nnz,
                       dev_out_coo, dev_dense_vec));
