...
int write_info(const char *name, const char *mat_name, int64_t rows,
               int64_t cols, int64_t nnz, bool symmetric, bool graph,
...
void compute_regularity(int64_t *csr_arow, int64_t rows, int64_t nnz,
  ...
  for (int64_t i = 0; i < rows; i++) {
    int64_t elements = csr_arow[i + 1] - csr_arow[i];
